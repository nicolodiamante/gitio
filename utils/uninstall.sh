...
echo "Starting the uninstallation process of gitio..."
...
if [[ -f "$ZSHRC" ]]; then
  ...
  # Prompt the user before removing gitio configurations.
  read -q "REPLY?Do you want to remove gitio configurations from .zshrc? [y/N] "
  echo ""
  if [[ "$REPLY" =~ ^[Yy]$ ]]; then
    # Use 'sed' to remove lines related to gitio.
...
echo "Uninstallation of gitio is complete."
